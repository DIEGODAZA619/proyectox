...
{
    var enlace = base_url + "Ingresos/Ingresos/cargarProveedores";
    $.ajax({ //
        url: enlace,
        method: 'POST',
...
function registrarIngreso()
{
    if (validardatosIngreso())
    {
        if(confirm('¿Estas seguro de confirmar el registro de material?'))
        {
            ...
            var enlace = base_url + "Ingresos/Ingresos/guardarRegistroIngreso";
...
function cargarTablaIngresos()
{
    ...
    var enlace = base_url + "Ingresos/Ingresos/cargarTablaIngresos";
...
{
   var enlace = base_url + "Ingresos/Ingresos/cargartablasMaterialesAcumulados";
...
{
    var enlace = base_url + "Ingresos/Ingresos/verificarMaterialSeleccionado";
    $.ajax({
        type: "POST",
        url: enlace,
...
function guardarIngreso()
{
    if (validardatos())
    {
        if(confirm('¿Estas seguro de confirmar el registro de material?'))
        {
            ...
            var enlace = base_url + "Ingresos/Ingresos/guardarIngresoMateriales";
...
function validardatos()
{
    ...
    if ($('#txtPrecio').val() == '') {
        ...
        alertaValidacion += " Precio Adquisición -  \n";
    }
    if ($('#txtPrecioVenta').val() == '') {
        todook = false;
        $('#txtPrecioVenta').closest(".form-group").addClass("has-error");
...
function eliminarIngresoDetalles(id_ingreso)
{
    ...
    {
        var enlace = base_url + "Ingresos/Ingresos/eliminarIngresoDetalles";
        $.ajax({
            type: "POST",
            url: enlace,
...
{
    limpiarFormularioIngreso();
    var enlace = base_url + "Ingresos/Ingresos/getIngresoDetalleEditarId";
    $.ajax({
        type: "POST",
        url: enlace,
        data: {id_ing: id_ingreso},
        success: function(data)
        {
            ...
            $.each(result, function(i, datos)
            {
                ...
                else
                {
                    ...
                    $('#txtCantidad').prop('disabled', false);
                    $('#txtPrecio').prop('disabled', false);
                    descripcionMaterial(datos.idve_producto);
                }
            });
        }
    });
...
function validardatosIngreso()
{
    ...
    $("  input[type=date]").each(function(){
        if($(this).attr('id') == 'txtfecha_factura'  )
        {
            if(this.value=='' )
            {
                    $(this).parent().addClass('form-group has-danger')
...
function aprobarIngreso()
{
    ...
    {   
        var enlace = base_url + "Ingresos/Ingresos/aprobarIngresoMateriales";        
        $.ajax({
            type: "POST",
            url: enlace,            
            success: function(data) 
            {
                ...
                $.each(result, function(i, datos)
                {
                    ...
                    {
                       swal({title: "CONFIRMADO",text: 'EL REGISTRO EN INVENTARIO FUE REGISTRADO CORRECTAMENTE...!!!',icon: "success",button: "OK",});
                        cargarfunciones();  
                        cargarMaterialesAcumulados();                     
                    }
                });
